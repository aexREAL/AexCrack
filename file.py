...
def menu(my_name,my_id):
	...
	io = '''[bold green]TELE:@REALaex\nWA:085892660928\nSTATUS: PENGEMBANGAN\nVERSI:BETA[bold green]'''
	oi = nel(io, style='green')
	cetak(nel(oi, title='[bold green] DEVELOPED AEX OFFCIAL [/bold green]'))
	wel=f'# CHANNEL :{kota}'
	cik2=mark(wel ,style='red')
	sol().print(cik2)
	print('')
	cetak(nel('1:[red]CRACK PUBLIC\n[white]2:[red]CRACK FILE\n[white]3:[red]CEK HASIL CRACK\n[white]4:[red]CP DERECTOR DI SINI\n[white]0:[red]out[blue] kelua/hapus kuki'))
	goro = input(f'\n{P}({H}?{P}) Chouse : ')
	print('')
	if goro in ['1']:
		dump_publik()
